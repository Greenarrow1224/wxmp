...
import com.green.mp.wechat.entity.MessageItem;
import com.green.mp.wechat.entity.WeChatCommon;
...
public class WeChatServiceImpl implements WeChatService {
    ...
    public String processRequest(HttpServletRequest request) {
        ...
        try {
            ...
            // 消息类型
            String msgType = (String) requestMap.get(WeChatCommon.MsgType);
            String mes = null;
            // 文本消息
            if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_TEXT)) {
                mes =requestMap.get(WeChatCommon.Content).toString();
                if(mes!=null&&mes.length()<2){
                    ...
                    Map<String, String> userInfo = getUserInfo(requestMap.get(WeChatCommon.FromUserName));
            ...
            // 图片消息
            else if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_IMAGE)) {
                respContent = "您发送的是图片消息！";
                respXml = WeChatUtil.sendTextMsg(requestMap, respContent);
            }
            // 语音消息
            else if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_VOICE)) {
                respContent = "您发送的是语音消息！";
                respXml = WeChatUtil.sendTextMsg(requestMap, respContent);
            }
            // 视频消息
            else if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_VIDEO)) {
                respContent = "您发送的是视频消息！";
                respXml = WeChatUtil.sendTextMsg(requestMap, respContent);
            }
            // 地理位置消息
            else if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_LOCATION)) {
                respContent = "您发送的是地理位置消息！";
                respXml = WeChatUtil.sendTextMsg(requestMap, respContent);
            }
            // 链接消息
            else if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_LINK)) {
                respContent = "您发送的是链接消息！";
                respXml = WeChatUtil.sendTextMsg(requestMap, respContent);
            }
            // 事件推送
            else if (msgType.equals(WeChatCommon.REQ_MESSAGE_TYPE_EVENT)) {
                // 事件类型
                String eventType = (String) requestMap.get(WeChatCommon.Event);
                // 关注
                if (eventType.equals(WeChatCommon.EVENT_TYPE_SUBSCRIBE)) {
                    respContent = "谢谢您的关注！";
                    respXml = WeChatUtil.sendTextMsg(requestMap, respContent);
                }
                // 取消关注
                else if (eventType.equals(WeChatCommon.EVENT_TYPE_UNSUBSCRIBE)) {
                    // TODO 取消订阅后用户不会再收到公众账号发送的消息，因此不需要回复
                }
                // 扫描带参数二维码
                else if (eventType.equals(WeChatCommon.EVENT_TYPE_SCAN)) {
                    // TODO 处理扫描带参数二维码事件
                }
                // 上报地理位置
                else if (eventType.equals(WeChatCommon.EVENT_TYPE_LOCATION)) {
                    // TODO 处理上报地理位置事件
                }
                // 自定义菜单
                else if (eventType.equals(WeChatCommon.EVENT_TYPE_CLICK)) {
                    // TODO 处理菜单点击事件
                }
            }
    ...
    private Map<String, String> getUserInfo(String s) {
        return null;
    }
}
